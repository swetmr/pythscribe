...
PythScribe compiles to JavaScript, but it does **not** inherit JavaScript's well-known semantic quirks. Where JS and Python disagree, the codegen emits the Python-faithful answer. This section enumerates which cases are handled by which mechanism. Exercised end-to-end by 15 dedicated codegen tests, the 1,432-test workspace suite, and the 1,318-entry CPython differential corpus.
...
Expected output: 1,432 Rust workspace + ~79 Node (runtime helpers 44, Worker-safe `core` 19, `web` 8, bigint 8) + 30 format-spec + 1,318 CPython diff + 8 fixture parity + 20 fuzz parity + 8 auto-route + 28 Playwright = **~2,900 green**. Everything in `RESULTS.md` is reproducible from `examples/cloudflare-bench/bench/`. `cargo build --workspace` emits zero warnings.
...
| CPython semantic differential | 1,318 | whole-program stdout matches CPython 3.12 (full corpus, `tests/differential/run.mjs`; cross-checked on a second JS engine) |
