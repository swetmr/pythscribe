...
PythScribe compiles to JavaScript, but it does **not** inherit JavaScript's well-known semantic quirks. Where JS and Python disagree, the codegen emits the Python-faithful answer. This section enumerates which cases are handled by which mechanism. Exercised end-to-end by 15 dedicated codegen tests, the 1,984-test workspace suite, and the 1,376-entry CPython differential corpus.
...
| `total // 0` | `ZeroDivisionError: division by zero` | `Error [ZeroDivisionError]: division by zero` | **Fully faithful** — codegen routes `//` and `%` through `pyFloorDiv` / `pyMod` always (CPython 3.14 unified every division/modulo zero message to `division by zero`; see `docs/python-oracle-policy.md`) |
...
| CPython semantic differential | 1,376 | whole-program stdout matches CPython 3.14.7 (full corpus, `tests/differential/run.mjs`; cross-checked on a second JS engine) |
