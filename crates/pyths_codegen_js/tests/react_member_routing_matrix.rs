...
fn assert_error_free(source: &str) {
    ...
    assert!(
        errs.is_empty(),
        "unexpected diagnostics for {source:?}: {errs:?}"
    );
}
...
fn unknown_members_get_from_import_parity_camel() {
    ...
    assert!(
        !js.contains("use_whatever_custom"),
        "no snake survivor:\n{js}"
    );
...
fn react_19_removed_members_are_compile_errors() {
    ...
    for (imp, base, member, js_name) in cases {
        for src in [
            ...
            assert!(
                errs.iter()
                    .any(|e| e.contains(js_name) && e.contains("React 19")),
...
fn removed_from_imports_diagnosed_on_core_packages() {
    let cases: &[(&str, &str)] = &[
        ...
        (
            "from react_dom import unmount_component_at_node",
            "unmountComponentAtNode",
        ),
...
fn ecosystem_namespace_members_camel_like_from_imports() {
    ...
    assert!(
        !js.contains("create_browser_router"),
        "no snake survivor:\n{js}"
    );
