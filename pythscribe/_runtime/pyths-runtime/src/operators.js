...
import {
    ...
    // #479: list `+=` extends from ANY iterable (list.__iadd__ === extend);
    // pyForIter is the canonical any-iterable adapter (raises CPython's
    // TypeError on a non-iterable). Call-time dereference keeps the cycle safe.
    pyForIter,
...
export function pyFloat(x) {
    // #490: nullary `float()` is 0.0 (CPython's default). Keyed on argument
    // COUNT — `float(None)` (null) and `float(x?.m())` (undefined) both pass
    // one argument and must raise; only a true zero-arg call is 0.0.
    if (arguments.length === 0) return __pyF(0);
...
export function pyInt(x, base) {
    // #490: nullary `int()` is 0 (CPython's default). Keyed on the ARGUMENT
    // COUNT, not on `x === undefined`: `int(None)` (null) and `int(x?.m())`
    // (an optional-chain short-circuit → undefined, still Python None) both
    // pass ONE argument and must raise, so only a true zero-arg call is 0.
...
// result keeps its bytearray-ness but not its identity).

/** Python `+=` — a list extends IN PLACE from ANY iterable (list/tuple/str/
 * set/dict-keys/generator), exactly like `list.__iadd__ === list.extend`;
 * a non-iterable rhs raises TypeError; immutables rebind via pyAdd. */
export function pyIAdd(a, b, fctx) {
    ...
    if (Array.isArray(a) && !a.__pytuple__) {
        // #479: list.__iadd__ IS list.extend — accept every iterable, not
        // just list/tuple. pyForIter raises CPython's TypeError on a true
        // non-iterable (`lst += 5` → "'int' object is not iterable"). The
        // snapshot makes a self-extend (`x += x`) terminate — CPython's
        // extend copies — and the in-place push keeps aliasing (`y = x;
        // x += (1,2)` leaves `x is y` True).
        const items = [...pyForIter(b)];
...
export function pyStr(obj) {
    // #490: nullary `str()` is '' (CPython's default). Keyed on argument
    // COUNT, NOT `obj === undefined`: `str(None)` (null) AND `str(x?.m())`
    // (an optional-chain short-circuit → undefined, still Python None) must
    // render 'None' via pyRepr below, so only a true zero-arg call is ''.
