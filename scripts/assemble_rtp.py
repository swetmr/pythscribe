...
def json_api_base(repository_url: str) -> str:
    """The Warehouse JSON-API origin for an upload (legacy) URL: `https://test.pypi.org/legacy/`
    -> `https://test.pypi.org`."""
    u = urllib.parse.urlsplit(repository_url)
    return f"{u.scheme}://{u.netloc}" if u.scheme and u.netloc else repository_url.rstrip("/")


def registry_files_via_json(repository_url: str, version: str, *, project: str = PROJECT, timeout: int = 60) -> dict[str, str]:
...
def assemble(manifest: dict, dist: Path, repository_url: str, env: dict[str, str],
    ...
    if registry_files is None:
        try:
            fetcher = fetch_registry or registry_files_via_json
            registry_files = fetcher(repository_url, manifest.get("version", ""))
    ...
    return {
        ...
        "producer": {
            "workflow": "publish-pypi.yml", "job": "testpypi",
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"R-TP: {rec['verdict']} -> {out} (uploaded {len(rec['uploaded_files'])}, registry-served {len(rec['registry_files'])})"
