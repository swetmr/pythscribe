...
// #478: a GLOBAL-registry symbol (Symbol.for), not a per-copy Symbol(). The
// marked kwargs carrier must be recognizable across module-copy boundaries —
// in `pyths run` the runtime is INLINED into the user module while stdlib
// modules (functools.partial) load from the materialized package, so an
// inlined __pyMarkKw and a materialized __pyTakeKw would otherwise hold
// distinct Symbol instances and never match. Same-copy usage is unchanged.
const __PYKW_MARK = Symbol.for("pyths.kwargs");
